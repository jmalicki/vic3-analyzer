...
async fn stub_use_save_then_active_queries() {
    ...
    let countries = eng
        .query("SELECT name FROM active.countries WHERE name = 'GER'")
        .await
    ...
    let constructions = eng
        .query("SELECT queue, building_type_name FROM constructions")
        .await
...
async fn selectors_and_latest_views_do_not_mutate_active() {
    ...
    let latest_countries = eng
        .query("SELECT name FROM latest.countries WHERE name = 'GER'")
        .await
