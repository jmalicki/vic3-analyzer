...
fn toy_economy_solve_converges() {
    ...
    assert!(world.buildings.iter().any(|b| b.type_id == wheat));
    assert!(world.buildings.iter().any(|b| b.type_id == bakery));
    ...
    assert!(result.goods.iter().any(|g| g.id == "wheat"));
    assert!(result.goods.iter().any(|g| g.id == "flour"));
    assert!(result.goods.iter().any(|g| g.id == "bread"));
}
...
fn what_if_extra_farm_levels_raises_wheat_supply_and_lowers_price() {
    ...
    assert_eq!(
        world
            .buildings
            .iter()
            .find(|b| b.type_id == defs.building_index_of("building_wheat_farm").unwrap())
            .map(|b| b.level),
        Some(3.0),
