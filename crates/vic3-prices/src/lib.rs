...
mod tests {
    ...
    fn state_and_building_detail_uses_pm_io_and_local_prices() {
        ...
        assert!(result
            .state_goods
            .iter()
            .any(|row| row.state_id == 7 && row.good_name == "wood" && row.sell == 3.0));
    }
    ...
    fn state_prices_follow_local_attributed_orders() {
        ...
        let wood = |state_id| {
            result
                .state_goods
                .iter()
                .find(|row| row.state_id == state_id && row.good_name == "wood")
    ...
    fn wage_pops_shop_at_local_prices_not_world() {
        ...
        let local_wood = result
            .state_goods
            .iter()
            .find(|row| row.state_id == 1 && row.good_name == "wood")
            .expect("local wood");
        assert_eq!(buyer.inputs[0].good_name, "wood");
        assert!(
