...
3. Prefer tool `campaign_brief` for a compact session / shortages / alert-kind summary, then query fact tables / TVFs as needed (`alerts()` is player-scoped; use `alerts('all')` for the full save).
...
impl ServerHandler for Vic3McpServer {
    fn get_info(&self) -> ServerInfo {
        ...
        .with_instructions(
            "Vic3 Analyzer MCP: discover saves → use_save → campaign_brief (optional) → query read-only SQL. \
