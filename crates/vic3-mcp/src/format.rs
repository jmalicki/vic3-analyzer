//! RecordBatch → MCP `query` payload (`json` | `csv`).
//!
//! JSON shape is normative in `docs/mcp.md`:
...
/// Convert collected batches into the MCP JSON result object.
pub(crate) fn batches_to_json(batches: &[RecordBatch]) -> Result<Value, FormatError> {
...
pub(crate) fn batches_to_csv(batches: &[RecordBatch]) -> Result<String, FormatError> {
    let (columns, rows) = batches_to_rows(batches)?;
