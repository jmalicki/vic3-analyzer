...
| `gdp` / `good_price` | ✅ | ✅ | Defs-based building candidates (first-of-type, state-scoped enqueue, no dominance prune) + Construction Sector meta lever + opt-in `SwitchPm` (`allow_pm_changes`) + price re-solves |
