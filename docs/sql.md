...
| `owner_tag` | Country tag (matches `countries.name`) |
| `market_id` | If modeled |
| `infrastructure` | As exposed in prices result |
| `arable_land` | If present |

**Storage today (Rust):** `World.states` / `PricesResult.states` are a **`Vec` of rows**, each with its own `id: u32`. Sparse Paradox ids. Save IR: `HashMap<u32, Option<State>>`. `StateInfo` stores `region_name` + `region_label` + `label`. SQL may build a **HashMap** for Exact `region_name` / `region_label` / `label` equality at bind (not range Exact).
...
Goods IO and PM recipes use **`List<Struct{good_name, good_label, qty}>`** (script id — never bare `GoodId` as the only key; required localized label; quantity):
...
| `input_goods` / `output_goods` | `List<Struct{good_name, good_label, qty}>` — script id (**not** raw `GoodId`), required localized label (always populated at emit), quantity. Project from resolved IO / `goods_io`. See nested-column unnest notes. |
...
-- SELECT region_label, label, owner_tag FROM world_states ORDER BY state_id LIMIT 20;
...
SELECT id, kind, title, good_name, state_id
FROM alerts()
WHERE severity = 1
LIMIT 40;
...
SELECT s.label, m.action, m.building, m.title
FROM states s
